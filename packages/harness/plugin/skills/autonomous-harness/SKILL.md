...
2. Run `baton-harness status <story>`.
...
`baton-harness worktree-create <story>` → JSON with `path` and `branch`.
...
Record an event: `baton-harness log-event <story> '{"phase":"start"}'`
...
Run `baton-harness verify unit` — holdouts should **FAIL** (RED). If they pass, the subagent didn't do its job; retry once.

**Antipattern validator (primitives only):**

```sh
baton-harness holdout-validate <story> --scope=story
```
...
Run (Bash, cwd = worktree): `baton-harness verify-all`.
...
Run `baton-harness holdout-check` from the worktree. Non-zero means the implementer touched a holdout; treat as a blocker.
...
- The diff (`baton-harness diff <story>`).
...
- `baton-harness result <story>` (optionally piping a custom body via stdin).
...
- Report to the user: branch name, attempt count, pointer to `baton-harness review <story>` for the HTML review page.
...
- Pipe it into `baton-harness handoff <story>`.
