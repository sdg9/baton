...
```sh
npx -y -p @baton-tools/harness@0.2.0 baton-harness <subcommand> [args]
```
...
2. Run `npx -y -p @baton-tools/harness@0.2.0 baton-harness status <story>`.
...
`npx -y -p @baton-tools/harness@0.2.0 baton-harness worktree-create <story>` → JSON with `path` and `branch`.
...
Record an event: `npx -y -p @baton-tools/harness@0.2.0 baton-harness log-event <story> '{"phase":"start"}'`
...
Run `npx -y -p @baton-tools/harness@0.2.0 baton-harness verify unit` — holdouts should **FAIL** (RED). If they pass, the subagent didn't do its job; retry once.

**Antipattern validator (primitives only):**

```sh
npx -y -p @baton-tools/harness@0.2.0 baton-harness holdout-validate <story> --scope=story
```
...
Run (Bash, cwd = worktree): `npx -y -p @baton-tools/harness@0.2.0 baton-harness verify-all`.
...
Run `npx -y -p @baton-tools/harness@0.2.0 baton-harness holdout-check` from the worktree. Non-zero means the implementer touched a holdout; treat as a blocker.
...
- The diff (`npx -y -p @baton-tools/harness@0.2.0 baton-harness diff <story>`).
...
- `npx -y -p @baton-tools/harness@0.2.0 baton-harness result <story>` (optionally piping a custom body via stdin).
...
- Report to the user: branch name, attempt count, pointer to `npx -y -p @baton-tools/harness@0.2.0 baton-harness review <story>` for the HTML review page.
...
- Pipe it into `npx -y -p @baton-tools/harness@0.2.0 baton-harness handoff <story>`.
...
```sh
npx -y -p @baton-tools/harness@0.2.0 baton-harness finish <story>
```
